...
pub async fn get_transaction_status(configuration: &configuration::Configuration, transaction_id: &str) -> Result<models::vendor_direct_fulfillment_transactions_2021_12_28::TransactionStatus, Error<GetTransactionStatusError>> {
    ...
    let uri_str = format!("{}/vendor/directFulfillment/transactions/2021-12-28/transactions/{transactionId}", configuration.base_path, transactionId=crate::apis::urlencode(p_transaction_id));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::TransactionStatus`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::TransactionStatus`")))),
        }
    } else {
        let content = resp.text().await?;
