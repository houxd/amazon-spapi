...
pub async fn get_customer_invoice(configuration: &configuration::Configuration, purchase_order_number: &str) -> Result<models::vendor_direct_fulfillment_shipping_2021_12_28::CustomerInvoice, Error<GetCustomerInvoiceError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_purchase_order_number = purchase_order_number;

    let uri_str = format!("{}/vendor/directFulfillment/shipping/2021-12-28/customerInvoices/{purchaseOrderNumber}", configuration.base_path, purchaseOrderNumber=crate::apis::urlencode(p_purchase_order_number));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::vendor_direct_fulfillment_shipping_2021_12_28::CustomerInvoice`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::vendor_direct_fulfillment_shipping_2021_12_28::CustomerInvoice`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn get_customer_invoices(configuration: &configuration::Configuration, created_after: String, created_before: String, ship_from_party_id: Option<&str>, limit: Option<i32>, sort_order: Option<&str>, next_token: Option<&str>) -> Result<models::vendor_direct_fulfillment_shipping_2021_12_28::CustomerInvoiceList, Error<GetCustomerInvoicesError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    ...
    let uri_str = format!("{}/vendor/directFulfillment/shipping/2021-12-28/customerInvoices", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::vendor_direct_fulfillment_shipping_2021_12_28::CustomerInvoiceList`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::vendor_direct_fulfillment_shipping_2021_12_28::CustomerInvoiceList`")))),
        }
    } else {
        let content = resp.text().await?;
